...
C        accessor, for each legacy COMMON cell the modern components
...
C        NO literal COMMON and no storage-association statement.
C
C     ROUTINE UNDER TEST   modern/state/stateacc.f
C        Paired GET/SET accessor SUBROUTINEs (one pair per wrapped
C        COMMON cell), each declared over an INCLUDE of the relevant
...
C
C     ZERO literal COMMON / ZERO aliasing  (the defining test/state
...
C        re-declared COMMON.  This driver goes further and uses NO
C        INCLUDE either -- it touches global state ONLY through the
C        accessor CALLs, so it is a clean witness of the zero-re-
C        declaration property it is testing.  Because COMMON /LOGOUT/
C        LOUT is therefore forbidden here, the verdict is written to the
C        LITERAL log unit 3 (NASTRAN's log unit -- the bootstrap sets
C        LOUT = 3 in bin/nastrn.f right after CALL DBMINT) -- the
C        identical destination without any COMMON.
C
...
C        depend on /LOGOUT/ LOUT being initialized.
C
C     HAPPY-PATH ONLY
C        Negative/error-path testing would need a MESAGE/DIAGLOG capture
C        stub backed by a COMMON block, which this suite's zero-COMMON
...
C=====================================================================
      PROGRAM TSTACC
C
C     ---- declarations (NO COMMON, NO aliasing) --------------------
      CHARACTER*80    LOG
...
C     (<= 6 chars; plain text, NOT COMMON references).
...
C     no-INCLUDE substitute for a direct-COMMON peek and is strictly
